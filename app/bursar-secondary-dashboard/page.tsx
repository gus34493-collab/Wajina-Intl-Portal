...
export default function BursarSecondaryDashboard() {
    ...
    async function fetchData() {
      try {
        const [statsRes, reqRes] = await Promise.all([
          fetch("/api/finance/dashboard-stats?campus=SECONDARY"),
          fetch("/api/requests?campus=SECONDARY&status=PENDING&limit=5"),
        ]);
        if (statsRes.ok) {
        ...
        console.error("Bursar Secondary fetch failed:", err);
      } finally {
        setLoading(false);
      }
    }
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-brand-primary/60 text-sm font-medium mt-1">Strategic financial oversight and structural transaction monitoring for Secondary Education.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-3 card p-10">
            <div className="flex justify-between items-center mb-8">
              ...
              <span className="text-token-micro font-black text-brand-tertiary uppercase tracking-wider">Secondary Campus</span>
            </div>
            {requests.length === 0 ? (
