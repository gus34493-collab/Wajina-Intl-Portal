...
const ADMIN = requireRole('DIRECTOR', 'PRINCIPAL', 'HEAD_TEACHER', 'VP_ADMIN');
...
router.get('/sessions', auth, async (req: any, res, next) => {
  try {
...
router.post('/sessions', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'CREATE', 'AcademicSession', session.id, name, req.ip);
    res.status(201).json(session);
  } catch (err) { next(err); }
});

// PATCH /api/structure/sessions/:id
router.patch('/sessions/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'UPDATE', 'AcademicSession', session.id, name || session.name, req.ip);
    res.json(session);
  } catch (err) { next(err); }
});

// DELETE /api/structure/sessions/:id
router.delete('/sessions/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'DELETE', 'AcademicSession', req.params.id, existing.name, req.ip);
...
router.get('/terms', auth, async (req: any, res, next) => {
  try {
...
router.post('/terms', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'CREATE', 'Term', term.id, `${name} - ${term.session.name}`, req.ip);
    res.status(201).json(term);
  } catch (err) { next(err); }
});

// PATCH /api/structure/terms/:id
router.patch('/terms/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'UPDATE', 'Term', term.id, term.name, req.ip);
    res.json(term);
  } catch (err) { next(err); }
});

// DELETE /api/structure/terms/:id
router.delete('/terms/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'DELETE', 'Term', req.params.id, existing.name, req.ip);
...
router.get('/classes', auth, async (req: any, res, next) => {
  try {
...
router.post('/classes', auth, ADMIN, async (req: any, res, next) => {
  try {
    const { name, campus, category, displayOrder } = req.body;
    if (!name || !campus) return res.status(400).json({ error: 'name and campus are required' });
    ...
    if (!cls) {
      ...
      audit(req.user.id, 'CREATE', 'Class', cls.id, `${campus} - ${name}`, req.ip);
    }
...
router.patch('/classes/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'UPDATE', 'Class', cls.id, cls.name, req.ip);
    res.json(cls);
  } catch (err) { next(err); }
});

// DELETE /api/structure/classes/:id
router.delete('/classes/:id', auth, requireRole('DIRECTOR'), async (req: any, res, next) => {
  try {
    const existing = await prisma.class.findUnique({ where: { id: req.params.id } });
    if (!existing) return res.status(404).json({ error: 'Class not found' });

    await prisma.class.delete({ where: { id: req.params.id } });
    audit(req.user.id, 'DELETE', 'Class', req.params.id, existing.name, req.ip);
...
router.get('/arms', auth, async (req: any, res, next) => {
  try {
...
router.post('/arms', auth, ADMIN, async (req: any, res, next) => {
  try {
    const { classId, label, teacherId, capacity } = req.body;
    if (!classId || !label) return res.status(400).json({ error: 'classId and label are required' });
    ...
    audit(req.user.id, 'CREATE', 'ClassArm', arm.id, arm.fullName, req.ip);
    res.status(201).json(arm);
  } catch (err) { next(err); }
});

// PATCH /api/structure/arms/:id
router.patch('/arms/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    ...
    audit(req.user.id, 'UPDATE', 'ClassArm', arm.id, arm.fullName, req.ip);
    res.json(arm);
  } catch (err) { next(err); }
});

// PATCH /api/structure/arms/:id/timetable - Form Teacher or Admin
router.patch('/arms/:id/timetable', auth, async (req: any, res, next) => {
  try {
    ...
    if (!arm) return res.status(404).json({ error: 'Class arm not found' });
    ...
    audit(userId, 'TIMETABLE_UPDATE', 'ClassArm', id, `Timetable updated for ${arm.fullName}`, req.ip);
    res.json({ message: 'Timetable updated successfully', timetableUrl: updated.timetableUrl });
  } catch (err) { next(err); }
});

// DELETE /api/structure/arms/:id
router.delete('/arms/:id', auth, ADMIN, async (req: any, res, next) => {
  try {
    const existing = await prisma.classArm.findUnique({ where: { id: req.params.id } });
    if (!existing) return res.status(404).json({ error: 'Class arm not found' });

    await prisma.classArm.delete({ where: { id: req.params.id } });
    audit(req.user.id, 'DELETE', 'ClassArm', req.params.id, existing.fullName, req.ip);
...
router.get('/overview', auth, async (req: any, res, next) => {
  try {
...
// ?teacherId=   → subjects taught by that teacher
// ?classId=     → subjects for that class
// No parameter  → subjects for the logged-in teacher (if FORM_TEACHER/TEACHER)
router.get('/subjects', auth, async (req: any, res, next) => {
  try {
    ...
    // Logic: 
    // 1. If teacherId provided, use it.
    // 2. If no teacherId, but user is a teacher, use their ID.
    // 3. If user is management and no filters, show all.
    
    if (teacherId) {
      where.teacherId = teacherId;
    } else if (['TEACHER', 'ACADEMIC_STAFF'].includes(role)) {
